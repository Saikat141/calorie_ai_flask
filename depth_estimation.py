...
class DepthEstimator:
    ...
    def load_depth_pro(self):
        ...
        try:
            # Import depth_pro here to handle cases where it might not be installed
            try:
                import depth_pro
                logger.info("Depth Pro package found, loading model...")
                self.model, self.transform = depth_pro.create_model_and_transforms()
                self.model.eval()
                self.model.to(self.device)
                logger.info(f"✓ Depth Pro model loaded successfully on {self.device}")
            except ImportError as e:
                logger.warning("⚠ Depth Pro not available. Using improved fallback depth estimation.")
                logger.info("To install Depth Pro:")
                logger.info("1. git clone https://github.com/apple/ml-depth-pro.git")
                logger.info("2. cd ml-depth-pro && pip install -e .")
                logger.info("3. Download models: wget https://ml-site.cdn-apple.com/models/depth-pro/depth_pro.pt -O checkpoints/depth_pro.pt")
                self.model = None
                self.transform = None
            except Exception as model_error:
                logger.error(f"⚠ Depth Pro model loading failed: {model_error}")
